package com.landmuc.kmmtranslator.android.voice_to_text.data
...
class AndroidVoiceToTextParser(
    ...
    override fun reset() {
        // avoid possible race condition with _state.update{it.copy()}
    ...
    override fun onRmsChanged(rmsDb: Float) {
        // normalize values from api to values between 0-1
        _state.update { it.copy(
    ...
    override fun onError(code: Int) {
        if (code == ERROR_CLIENT) { return }
        _state.update { it.copy(error = "Error: $code") }
    }

    override fun onResults(result: Bundle?) {
        result
            //actual text of voice input
            ?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)
            //relevant result is at the first index
            ?.getOrNull(0)
            ?.let { text ->
                _state.update { it.copy(
                    result = text
